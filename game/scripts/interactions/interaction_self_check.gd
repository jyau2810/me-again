...
func _test_scene_layout_store() -> void:
	...
	_expect(headlight.get("visual_size", Vector2.ZERO) == Vector2(157.0, 45.0), "headlight review overlay keeps its source-derived visual size")
	...
	_expect(headlight.get("source_rect", []) == [562, 695, 205, 59], "headlight retains its review-master extraction coordinates")
	_expect(headlight.get("state_asset_paths", {}).size() == 2, "headlight exposes neutral and blink review states")
	_expect(LayoutStore.target_asset_path("c01_s02_commute_window", "headlight", "blink").ends_with("prop_vehicle_headlights_blink_v001.png"), "headlight blink state resolves its review asset")
	...
	_expect(plate.get("visual_size", Vector2.ZERO) == Vector2(71.0, 27.0), "plate review overlay keeps its source-derived visual size")
	_expect(plate.get("hit_size", Vector2.ZERO) == Vector2(96.0, 56.0), "plate keeps a forgiving hit rectangle around the small visual")
	_expect(plate.get("source_rect", []) == [635, 742, 93, 35], "plate retains its review-master extraction coordinates")
	_expect(plate.get("state_asset_paths", {}).size() == 2, "plate exposes neutral and mouth-hint review states")
	_expect(LayoutStore.target_asset_path("c01_s02_commute_window", "plate", "mouth_hint").ends_with("prop_vehicle_plate_mouth_hint_v001.png"), "plate mouth-hint state resolves its review asset")
...
func _test_scene_visual_composer() -> void:
	...
	_expect(headlight_view != null and headlight_view.size == Vector2(157.0, 45.0), "runtime composer instantiates the calibrated headlight review overlay")
	_expect(headlight_view.z_index < composer.layer_view("vehicle_glass_rain").z_index, "headlight review overlay renders below the vehicle rain texture")
	_expect(composer.set_target_state("headlight", "blink"), "runtime composer switches the headlight to its restrained blink review state")
	...
	_expect(composer.set_target_state("plate", "mouth_hint"), "runtime composer switches the plate to its mouth-hint review state")
