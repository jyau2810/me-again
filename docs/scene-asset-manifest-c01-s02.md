...
- 状态：`candidate_review`；当前车道线、车辆尺度、透视与接地基准，不是运行时背景或独立车身资产。
...
- 完整关系预览：`preview_c01_s02_vehicle_focus_context_look_up_v004.png` 只叠加已确认的人物、手机和结构层，用于检查人物视线、遮挡和焦点关系。
- 后续拆分：用户确认此场景基准后，才派生 `prop_vehicle_focus_base_v004_candidate.png`、独立路面阴影/湿反射和独立玻璃雨纹效果；三者分别落位并保持可人工校正。
...
初始值仅用于迁移当前交互契约，不视为视觉确认结果。v004 独立车身派生后，`headlight` 与 `plate` 必须按实际灯罩和前牌重新校正；正式构图与资产进入 Godot 后，全部通过校正工具人工调整。
...
| `focus_vehicle_base` | 将由 v004 场景基准派生 | 待派生 | 待派生 | 1 | v001、v002 已拒绝；v003 仅保留车辆尺度，v004 场景确认后再建立未锁定布局 |
...
7. 手机已确认；当前确认 v004 的焦点车完整场景关系，重点检查用户标注的车道边界轨迹，确认后再拆分车辆并校准车灯/车牌位置。
