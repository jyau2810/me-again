...
- 状态：`candidate`
- 位置：`../game/assets/art/style-studies/c01_s02/foreground/`；确认前不复制到 production，也不接入运行时。
- 内容：与正式背景同画布、同坐标，只保留空座前方的水平旧黄扶手、底部弧形旧黄扶手和最前排座椅上缘；其余区域透明。
- 合成职责：绘制在成年人物、手机和局部效果层之上，以遮住人物下半身和可能越过扶手的手部；固定遮挡不硬裁进人物 PNG，人物仍可人工移动校正。
- 技术检查：941 × 1672 RGBA PNG；可见覆盖率约 9.64%，透明边缘有效，强不透明像素与正式背景同坐标平均色差 5.31 / 255，合成未见漂移。
- 待确认：两段扶手的保留范围、底部座椅上缘高度、边缘是否干净，以及人物可移动区是否足够；确认后晋级为 `production/c01_s02_commute_window/foreground/fg_c01_s02_bus_rail_occluder_v001.png`。
...
4. 确认 `fg_c01_s02_bus_rail_occluder_v001_candidate.png` 的保留范围、透明边缘与同坐标覆盖关系，确认后晋级正式前景遮挡层。
5. 前景遮挡层通过后生成成年主角低头坐姿，并按固定座椅调整骨盆、背部、手部及前景遮挡关系。
