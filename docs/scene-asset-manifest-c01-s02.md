...
- 状态：`candidate`
- 位置：`../game/assets/art/style-studies/c01_s02/`；确认前不进入 `production/`。
...
- 待确认：正式完成度、冷暖与暗部可读性、雨滴密度、座椅与扶手材质、右侧暗红车方向，以及三个叠加预留区。

#### `bg_c01_s02_bus_night_base_v001.png`

- 状态：`planned`
- 晋级规则：仅在上方候选确认后复制到 `production/c01_s02_commute_window/background/`，不直接用候选文件充当正式资产。
...
3. 确认 `bg_c01_s02_bus_night_base_v001_candidate.png` 的正式完成度、材质、光色、暗部可读性与分层边界，确认后晋级正式背景。
4. 依次确认成年主角、儿童倒影、手机和焦点车辆。
5. 确认车灯、车牌与反光状态图。
6. 完成静态和四状态合成预览。
7. 在 Godot 中人工校正布局并保存 JSON。
8. 接入交互、声音和状态切换，完成单幕验收。
