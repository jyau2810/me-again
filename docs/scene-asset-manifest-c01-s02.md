...
#### `char_child_reflection_curious_v001_candidate.png`

- 状态：`candidate_review`；位于 `../game/assets/art/style-studies/c01_s02/characters/`，确认前不进入 production 或布局 JSON。
- 内容：同一人物约五岁时的儿童版本，身体朝车前方，头部转向左侧看回成年人；穿儿童日常圆领上衣，空手，以嘴唇微张、松弛眉眼和自然关注表达轻快感。
- 文件：`762 × 1484` 紧边界 RGBA，四边保留透明安全区；正常不透明度底图不烘焙玻璃、雨纹、场景光或反射强度。候选 SHA-256 为 `34881d7addb179e1c01c837c6c540ee006feaeca1a743a3197d92e481d8239f1`。
- 评审落位：原始画布 `rect = [574, 875, 294, 572]`，对应逻辑建议 `anchor = (0.7662, 0.6944)`、`visual_size = 225 × 438`；`preview_c01_s02_child_reflection_v001.png` 使用 22% Alpha、低饱和/对比和轻微柔化，并以玻璃内轮廓硬裁切。
- 技术检查：候选四角 Alpha 为 0，没有可见绿边；预览差异只位于 `(604, 881)–(862, 1323)`，玻璃遮罩外差异像素为 0。推荐落位仍待视觉确认，确认前不晋级、不接入运行时。
- 回归检查：Godot 4.7.1 正常导入候选与预览；状态与存档 138 条断言、内容目录 5 章 30 幕 9 件收集物、交互系统 237 条断言与 17 类契约通过，主场景与校正器 smoke test 正常。
...
- 当前边界：车辆完整场景、初始分层和车灯/车牌状态 Gate 已关闭；儿童倒影仍处于独立候选 Gate。
...
9. 约五岁儿童倒影 v001 独立候选和 22% 弱反射整场预览已生成，当前确认造型、镜像朝向、表情、落位和玻璃裁切；确认后生成冷暖效果，所有层在校正工具中人工调整并保存，再接入三次观察状态。
