...
- 状态：`candidate_review`；当前直线车道、车辆尺度、透视与接地基准，不是运行时背景或独立车身资产。
...
- 后续拆分：用户确认此场景基准后，才派生 `prop_vehicle_focus_base_v005_candidate.png`、独立路面阴影/湿反射和独立玻璃雨纹效果；三者分别落位并保持可人工校正。
...
| `focus_vehicle_base` | 将由 v005 场景基准派生 | 待派生 | 待派生 | 1 | v001、v002 已拒绝；v003 仅保留车辆尺度，v004 为曲线反例，v005 场景确认后再建立未锁定布局 |
...
7. 手机已确认；当前确认 v005 的焦点车完整场景关系，重点检查用户标注通道内的直线车道边界，确认后再拆分车辆并校准车灯/车牌位置。
