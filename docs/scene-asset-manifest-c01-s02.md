...
- 初始落位：`anchor = (0.1775, 0.5105)`，`visual_size = 256 × 77`，`z_index = 7`；由母版坐标计算，可在校准器中独立拖动、缩放和锁定。
...
- 初始落位：`anchor = (0.6318, 0.9025)`，`visual_size = 530 × 250`，`z_index = 7`；由母版坐标计算，可在校准器中独立拖动、缩放和锁定。

两张 production 资产合并回母版坐标后，Alpha 差异为 0，可见 RGBA 像素差异为 0；没有重绘、缩放或边缘损失。它们绘制在成年人物、手机和局部效果层之上，固定遮挡不硬裁进人物 PNG。
...
| --- | --- | --- | --- | --- | --- |
| `seat_armrest_occluder` | `fg_c01_s02_seat_armrest_occluder_v001.png` | `(0.1775, 0.5105)` | `256 × 77` | 7 | 可独立校正，未锁定 |
| `front_seat_occluder` | `fg_c01_s02_front_seat_occluder_v001.png` | `(0.6318, 0.9025)` | `530 × 250` | 7 | 可独立校正，未锁定 |
...
4. 已将全画布遮挡母版改为 `reference_only`，并拆出 `seat_armrest` 与 `front_seat` 两张紧边界 production 资产；在校准器确认各自初始落位和可移动范围后锁定。
5. 两个前景层落位通过后生成成年主角低头坐姿，并按固定座椅调整骨盆、背部、手部及前景遮挡关系。
