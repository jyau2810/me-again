...
- `reference_id`：`candidate_c01_s02_vehicle_focus_context_02`
- `reference_type`：待确认的场景内车辆透视与接地基准
- `reference_purpose`：先确认车身纵轴、车道占位、尺度、接地阴影、湿路反射、环境光和玻璃雨层，再派生独立资产
...
- 全画布 `fg_c01_s02_bus_rail_occluder_v001_candidate.png` 已降为提取母版；两张紧边界 production 结构层、成年主角低头/抬头状态与 `prop_phone_commute_cold_v001.png` 均已确认。`prop_vehicle_focus_base_v001_candidate.png` 已拒绝。当前只确认 `preview_c01_s02_vehicle_focus_context_v002.png`：检查角度、车道占位、尺度、接地阴影、破碎湿反射和玻璃雨层关系；确认前不派生独立 v002 车身或写入布局 JSON。
