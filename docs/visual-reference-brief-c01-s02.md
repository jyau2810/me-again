...
- `reference_id`：`candidate_c01_s02_vehicle_focus_context_03`
- `reference_type`：待确认的场景内车辆尺度、透视与接地基准
- `reference_purpose`：先确认车宽与车道宽比例、两侧湿路余量、车身纵轴、接地阴影、湿路反射、环境光和玻璃雨层，再派生独立资产
...
- 全画布 `fg_c01_s02_bus_rail_occluder_v001_candidate.png` 已降为提取母版；两张紧边界 production 结构层、成年主角低头/抬头状态与 `prop_phone_commute_cold_v001.png` 均已确认。车辆 v001 因角度和贴图感被拒绝，v002 因车宽超过车道被拒绝。当前只确认 `preview_c01_s02_vehicle_focus_context_v003.png`：重点检查车宽/车道宽比例和两侧湿路余量，同时复核角度、接地阴影、破碎湿反射和玻璃雨层；确认前不派生独立 v003 车身或写入布局 JSON。
