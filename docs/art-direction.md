...
状态：`approved` 视觉与构图基准；背景构图 v003 与正式无人物背景 v001 均已确认，当前进入前景遮挡层评审。
...
当前规范与状态见 [人物一致性包](./character-consistency-pack.md)。成年主角 v001 设定表已作为 `candidate` 归档，等待单项确认；确认前不生成儿童设定表，也不把候选用于 production 或运行时。

## 5. 场景规范

- 每幕拥有自己的场景构图，不再用一张章节主背景覆盖六幕。
- 背景只包含不可移动、不可替换的建筑和环境结构。
- 固定座椅、坐垫、靠背、扶手和人物接触面属于背景结构；移除独立人物时必须完整保留，不能用空侧板或无承载面的地板替代。
- 扶手与护栏必须位于座椅边界或上沿并留出可信乘坐净空，不得穿过靠背、坐垫、人物骨盆接触面或腿部空间。
- 与可移动人物发生前后关系的固定扶手与前排座椅上缘必须按实际内容裁成独立透明结构层；空间上不相连的结构分别成图，记录原始裁切矩形与锚点，并通过场景合成确定各自前后层级。人物后方的扶手保持在人物下层，真正遮挡人物的前排座椅上缘保持在人物与手机上层。不得用大面积透明全画布文件充当可摆放物件，也不得把固定结构硬裁进人物 PNG。
