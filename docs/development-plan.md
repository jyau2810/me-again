...
- `../game/assets/art/style-studies/c01_s02/props/prop_vehicle_focus_base_v001_candidate.png`：普通无品牌深色家用车的紧边界独立候选，近正面驶向公交。
- `../game/assets/art/style-studies/c01_s02/previews/preview_c01_s02_vehicle_focus_base_v001.png`：焦点车辆候选在正式背景空置对向车道的落位评审图。
- `../game/assets/art/style-studies/c01_s02/previews/preview_c01_s02_vehicle_focus_base_look_up_v001.png`：焦点车辆候选与已确认人物、手机和结构层的完整关系评审图。
...
- Stage 3 继续保持“进行中”；成年主角低头、抬头和独立手机 Gate 均已关闭。当前只评审独立对向焦点车辆 v001 的车型、迎面方向、场景比例、透视和落位，不提前生成车灯、车牌、儿童倒影或扩展其他场景。
...
执行 Stage 3：两个紧边界 production 结构层、约 26 岁成年主角的低头/抬头状态和独立手机均已确认；人物状态共用一个锁定落位，手机使用 production 路径并保持可人工校正。当前评审 `prop_vehicle_focus_base_v001_candidate.png`：检查普通无品牌家用车身份、对向迎面方向、道路透视、场景比例和初始落位 `(469, 588, 400, 294)`；确认后才晋级 production 并以未锁定布局接入校正器，确认前不生成车灯、车牌、儿童倒影或扩展到其余场景。
