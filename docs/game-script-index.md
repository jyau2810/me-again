...
- 交互系统：290 条断言、17 类契约自检通过，包含 JSON 场景背景、完整画布热点、八张运行时视觉层、跨层手机目标视觉、已迁移目标的纯命中 Board 模式、人物/车灯/车牌/儿童/暖意状态切换、逐次观察反馈、可选热点解锁、倒影样式与玻璃遮罩、车辆分层、裁切坐标、雨纹层级、旧布局回退和视觉/命中尺寸分离检查。
